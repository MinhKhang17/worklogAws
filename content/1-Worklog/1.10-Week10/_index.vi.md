...
* **Backend**:
  * Nắm được toàn bộ luồng tạo artifact cho container: tối ưu hóa quá trình build Docker, gắn thẻ image bất biến (immutable) và publish image backend lên Amazon ECR.
  ...
  * Học các mô hình triển khai an toàn như rolling updates và phần trăm healthy tối thiểu để giảm thiểu rủi ro khi release.
  * Thực hành tách biệt cấu hình runtime khỏi chính image thông qua các biến môi trường cấp độ task và tiêm chuỗi bảo mật (secret injection).
...
* **Frontend**:
  * Tìm hiểu kiến trúc phân phối frontend sử dụng dịch vụ lưu trữ tĩnh S3 kết hợp với CloudFront để caching toàn cầu và giảm độ trễ.
  * Nghiên cứu thiết lập HTTPS từ đầu đến cuối: cấp phát chứng chỉ ACM, quy trình xác thực và định tuyến DNS Route 53.
  * Hiểu mô hình Kiểm soát Truy cập Nguồn gốc (Origin Access Control) để S3 có thể giữ ở trạng thái private trong khi CloudFront phục vụ lưu lượng người dùng một cách an toàn.
  * Học về khóa cache (cache-key) và chiến lược vô hiệu hóa (invalidation) nhằm cân bằng giữa việc cung cấp UI mới nhất với hiệu suất cao của CDN.
  ...
  * Thực hành tư duy phát hành frontend với việc xóa cache (cache busting), kiểm soát lan truyền và phiên bản hóa tài nguyên thân thiện với việc rollback.
