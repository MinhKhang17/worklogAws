...
| 4   | - Build **mediaService** (Frontend) <br>&emsp; + `getImageUrl(owner, id)` — fetch image URL from `GET /api/worklogAws/images/{owner}/{id}` <br>&emsp; + In-memory URL cache + in-flight deduplication (prevents duplicate API calls for the same image) <br>&emsp; + Bulk helpers: `getFoodImageUrlMap(ids[])`, `getWorkoutPlanImageUrlMap(ids[])`, `getExerciseImageUrlMap(ids[])` | 02/26/2026 | 02/26/2026 | |
...
* **Backend — Media module**:
  ...
  * `GET /api/worklogAws/images/exercise/{id}` and similar endpoints return all images for a given entity.
