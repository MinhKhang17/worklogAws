---
title : "Lab 3: EMR Iceberg - using Iceberg tables"
date : 2026-03-16 
weight : 5 
chapter : false
pre : " <b> 5.5. </b> "
---


In this lab, for self paced labs we will create an EMR cluster with Release 6.5 with Iceberg installed. For AWS Events the EMR cluster is pre-created. This lab has been tested with `EMR 6.5.0`. Hence, we recommend that you use the same EMR release for your workshop.

We will use PySpark Jupyter Notebook to work interactively with Iceberg tables.

For self paced labs, before you begin this lab make sure you have completed **Create S3 bucket** Lab. For AWS events the S3 bucket is pre created.

